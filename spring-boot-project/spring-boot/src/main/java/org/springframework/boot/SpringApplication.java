...
public class SpringApplication {
	...
	public SpringApplication(ResourceLoader resourceLoader, Class<?>... primarySources) {
		...
		// 这里仅仅是获取当前工程下的 spring.factories，如果工程引入了一些三方的依赖，会有一些其他的初始化器
	...
	/**
	 * 在实例化一个 springApplication 对象之后，就开始调用 run() 方法
	 * <p>
	...
	public ConfigurableApplicationContext run(String... args) {
		...
		// 初始化器初始化 (在实例化 springApplication 对象的时候，获取到初始化器列表的值给到 bootstrapRegistryInitializers)
		DefaultBootstrapContext bootstrapContext = createBootstrapContext();
		ConfigurableApplicationContext context = null;
		// 设置 headless 模式
		configureHeadlessProperty();
		// 获取 springApplication 的监听器集合，并且把获取到的集合给到 SpringApplicationRunListeners 对象
		SpringApplicationRunListeners listeners = getRunListeners(args);   // 这里获取的是 META-INF/spring.factories 里面的
		// 启动开始监听器
